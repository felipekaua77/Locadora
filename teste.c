...
void salvar_filmes() {
    ...
    for (int i = 0; i < MAX_GENEROS; i++) {
        ...
        for (int j = 0; j < generos[i].num_filmes; j++) {
            ...
            fprintf(file, "%s\n%.2f\n%d\n", filme->nome_filme, filme->preco_filme, filme->disponivel);
        }
        fprintf(file, "END\n");
    }
...
// Função que carrega os filmes de um arquivo de texto
void carregar_filmes() {
    ...
    for (int i = 0; i < MAX_GENEROS; i++) {
        ...
        strcpy(generos[i].nome_genero, buffer);
        generos[i].num_filmes = 0;
        ...
        while (fgets(buffer, sizeof(buffer), file)) {
            ...
            Filme *filme = &generos[i].filmes[generos[i].num_filmes];
            strcpy(filme->nome_filme, buffer);
            fgets(buffer, sizeof(buffer), file);
            filme->preco_filme = atof(buffer);
            fgets(buffer, sizeof(buffer), file);
            filme->disponivel = atoi(buffer);
            generos[i].num_filmes++;
        }
    }
...
void cadastrar_filme() {
    ...
    if (genero_id >= 0 && genero_id < MAX_GENEROS && generos[genero_id].num_filmes < MAX_FILMES) {
        ...
        salvar_filmes(); // Salva os filmes no arquivo
...
int main() {
    srand(time(NULL));
    carregar_filmes(); // Carrega os filmes do arquivo ao iniciar o programa
    menu_filmes();
